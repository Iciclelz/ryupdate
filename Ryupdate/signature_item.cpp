...
signature_item::signature_item(QWidget* parent,
  ...
  type_widget->insertItems(0, QStringList({"Address", "Operator", "Operand 1",
                                           "Operand 2", "Operand 3"}));
  type_widget->setFont(QFont("Segoe UI", 8));
  type_widget->setCurrentIndex(this->type - 1);

  signature_widget = new QTableWidgetItem(
      QString::fromStdString(aobscan(signature).get_pattern()));
  ...
  connect(result_widget.get(),
          static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
          [this](int) { this->result = result_widget->value(); });
}

signature_item::~signature_item() {}

void signature_item::update_data(
    const std::pair<address_t, size_t>& scan_region) {
  void* pbase = reinterpret_cast<void*>(scan_region.first);
  size_t size = scan_region.second;

  if (pbase == 0 || size == 0) {
    auto getmodulesize = [](HMODULE module, void** pbase,
                            size_t* psize) -> bool {
      if (!module) {
        return false;
      }
      ...
      if (module == GetModuleHandle(NULL)) {
        PIMAGE_NT_HEADERS pimage_nt_headers =
            ImageNtHeader(reinterpret_cast<void*>(module));
  ...
  unsigned long address = aobscan(this->signature, pbase, size, this->result)
  ...
  if (type == type_address) {
    this->data_widget->setText(
        QString::fromStdString(uint_to_string<address_t>(address)));
  } else {
    disassembler disassemble(static_cast<address_t>(address),
                             z.readmemory(address, 12));
    std::vector<instruction> instructions = disassemble.get_instructions();

    if (type == type_operator) {
      this->data_widget->setText(
          QString::fromStdString(instructions.at(0).mnemonic).toUpper());
    } else {
      ...
      if (x.op_count < operand_index + 1) {
        ...
        std::function<QString(const cs_x86_op&, size_t)> get_operand_data =
            [&](const cs_x86_op& operand, size_t operand_index) -> QString {
          switch (operand.type) {
            case X86_OP_REG:
              return QString::fromUtf8(
                         cs_reg_name(disassemble.handle,
                                     x.operands[operand_index].reg))
                  .toUpper();

            case X86_OP_IMM:
              return QString::fromStdString(
                  hexadecimal_to_string(x.operands[operand_index].imm));

            case X86_OP_MEM:
              x86_op_mem mem = x.operands[operand_index].mem;

              printf(
                  "OPERATION_MEMORY_DETECTED (%s)\nBase: %d\tDisp: "
                  "%llX\tIndex: %d\tScale: %d\tSegment: %d\n\n",
                  this->name.c_str(), mem.base, mem.disp, mem.index, mem.scale,
                  mem.segment);

              if (mem.base != 0) {
                this->comments_widget->setText(
                    QString::fromUtf8(
                        cs_reg_name(disassemble.handle,
                                    static_cast<x86_reg>(mem.base)))
                        .toUpper() +
                    "+" +
                    QString::fromStdString(hexadecimal_to_string(mem.disp)));
              }
              if (mem.disp != 0) {
        ...
        //
        this->data_widget->setText(
            get_operand_data(x.operands[operand_index], operand_index));
      }
    }
  }
}
