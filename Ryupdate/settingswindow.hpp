...
class settingswindow : public QDialog {
 ...
 public:
  ...
  size_t insert(const std::vector<std::string>& n,
                const std::vector<treeviewitem_t>& items);

 private:
  QTreeWidgetItem* check_treewidgetitem(
      const std::string& n,
      const std::function<void(bool)>& handler = [](bool) {});

  std::unique_ptr<QHBoxLayout> layout;
  std::unordered_map<QTreeWidgetItem*, std::function<void(bool)>>
      check_treewidgetitem_handler;
