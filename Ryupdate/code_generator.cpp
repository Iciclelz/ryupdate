...
function::function(access_specifier access,
                   const std::pair<std::string, std::string>& decl) {
  this->access = access;
  this->decl = decl;
}
...
std::string function::get_header() {
  ...
  return decl.second + " " + decl.first + "(" +
         (parameters.size() == 0 ? ")" : argument.str());
}
...
std::string function::get_source(const std::string class_name) {
  ...
  text << decl.second << ' ' << class_name << "::" << decl.first << '('
       << (parameters.size() == 0 ? ")" : argument.str());
  text << "\n{\n" << impl.get_code() << "\n}\n";
  return text.str();
}

code_generator::code_generator(const std::string& class_name,
                               const implementation& constructor_impl,
                               const implementation& destructor_impl) {
...
void code_generator::insert_field(access_specifier access,
...
std::string code_generator::get_header() {
  ...
  for (int32_t i = 1; i < 4; ++i) {
    auto function_range =
        function_graph.equal_range(static_cast<function::access_specifier>(i));
    auto field_range = field_graph.equal_range(
        static_cast<code_generator::access_specifier>(i));

    if (function_range.first == function_range.second &&
        field_range.first == field_range.second && i != 1) {
      continue;
    }
